...
public class AchievementsMonographController extends BaseController
{
    ...
    {
        AchievementDataScopeUtils.applyAchievementListScope(achievementsMonograph, "monograph:monograph");
        startPage();
        List<AchievementsMonograph> list = achievementsMonographService.selectAchievementsMonographList(achievementsMonograph);
    ...
    public AjaxResult export(HttpServletResponse response,@RequestBody ExportRequestDTO<AchievementsMonograph> exportRequestDTO)
    {
        ...
        AchievementsMonograph query = exportRequestDTO.getData() == null ? new AchievementsMonograph() : exportRequestDTO.getData();
        AchievementDataScopeUtils.applyAchievementListScope(query, "monograph:monograph");
        List<AchievementsMonograph> list = achievementsMonographService.selectAchievementsMonographList(query);
        list = AchievementDataScopeUtils.filterByIds(list, ids, "getMonographId");
    ...
    {
        AchievementsMonograph achievementsMonograph = achievementsMonographService.selectAchievementsMonographByMonographId(monographId);
        if (!AchievementDataScopeUtils.canAccessAchievementRecord(achievementsMonograph, "monograph:monograph")) {
            return AjaxResult.error("无权限访问该数据");
        }
        return AjaxResult.success(achievementsMonograph);
    }
    ...
    {
        achievementsMonograph.setUserId(null);
        achievementsMonograph.setDeptId(null);
        achievementsMonograph.setCreatedAt(null);
        achievementsMonograph.setUpdatedAt(null);
        int rows = achievementsMonographService.updateAchievementsMonograph(achievementsMonograph);
        if (rows > 0) {
            AjaxResult result = AjaxResult.success("修改成功");
            result.put("monographId", achievementsMonograph.getMonographId());
            return result;
        }
